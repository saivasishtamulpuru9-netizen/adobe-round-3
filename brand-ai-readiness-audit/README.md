...
| `tgbie_bot_block.json` | `tgbienew.cgg.gov.in` | **0 findings** | 0 Strengths, 1 Flag-Only | **⚠️ Discrepancy Note:** Despite the filename `tgbie_bot_block.json`, the report records a successful fetch with 0 total findings and `overall_health: "excellent"` (with 1 `FS-05` flag-only item), showing no bot-block flags were triggered during this run. |
